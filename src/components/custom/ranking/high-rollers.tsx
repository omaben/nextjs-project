...
import {
   selectAuthBetsHighRollers,
   selectAuthCurrencyOption,
   selectAuthCurrentBrand,
   selectAuthOperator,
} from 'redux/authSlice'
import { selectLoadingHighRollers } from 'redux/loadingSlice'
import {
...
export default function HighRollers({
   ...
   const loadingHighRollers = useSelector(selectLoadingHighRollers)
   let dataSort: {
      betAmount?: number
      betAmountInEUR?: number
      betAmountInUSD?: number
   } = {
      betAmount: -1,
   }
   if (currencyOption.name === CURENCYTYPE.INEUR) {
      dataSort = {
         betAmountInEUR: -1,
      }
   }
   if (currencyOption.name === CURENCYTYPE.INUSD) {
      dataSort = {
         betAmountInUSD: -1,
      }
   }
   ...
   const data = useSelector(selectAuthBetsHighRollers)
