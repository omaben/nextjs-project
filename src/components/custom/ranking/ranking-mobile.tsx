...
import { StartEndDateProps, betListType, betListTypeText } from 'types'
...
export default function RankingMobile({
   ...
   const handleChange = (newValue: betListType) => {
      switch (newValue) {
   ...
   return (
      <Grid container key={searchDate}>
         <Grid item xs={12} textAlign={'center'}>
            ...
            >
               <FontAwesomeIcon
                  ...
                  size="sm"
                  onClick={() => {
                     const valueSelected = Object.keys(betListType).findIndex(
                        (item) => item === value
                     )
                     const valueIndex =
                        valueSelected === 1
                           ? Object.keys(betListType).length - 1
                           : valueSelected - 1
                     const currentValue = Object.keys(betListType).find(
                        (item, index) => index === valueIndex
                     )
                     handleChange(currentValue as betListType)
                  }}
               />
               <Box
                  sx={{
               ...
               >
                  {valueIcon}
                  <Typography variant="h5" color={ImoonGray[6]}>
                     {betListTypeText[value as betListType]}
                  </Typography>
               </Box>
               <FontAwesomeIcon
                  ...
                  onClick={() => {
                     ...
                     const valueIndex =
                        valueSelected === Object.keys(betListType).length - 1
                           ? 1
                           : valueSelected + 1
                     const currentValue = Object.keys(betListType).find(
                        (item, index) => index === valueIndex
         ...
         {value === betListType.LATESTWINS && (
            <Grid item xs={12}>
               <LastWinners startDate={startDate} endDate={endDate} />
            </Grid>
         )}
         {value === betListType.BIGWINS && (
            <Grid item xs={12}>
               <BigWins startDate={startDate} endDate={endDate} />
            </Grid>
         )}
         {value === betListType.BIGLOSSES && (
            <Grid item xs={12}>
               <BigLoser startDate={startDate} endDate={endDate} />
            </Grid>
         )}
         {value === betListType.HIGHROLLERS && (
            <Grid item xs={12}>
               <HighRollers startDate={startDate} endDate={endDate} />
            </Grid>
         )}
         {value === betListType.LUCKYWINS && (
            <Grid item xs={12}>
               <LuckyWins startDate={startDate} endDate={endDate} />
            </Grid>
         )}
      </Grid>
   )
