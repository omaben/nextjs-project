...
export default function PortalStatsLineChartReportsQuick(
   ...
   props.rows &&
      props.rows?.map((report: { from: number; uniquePlayers: number }) => {
   ...
   const updateChart = () => {
      ...
      switch (currentActiveChart) {
         ...
         case ChartDataName.PLAYER:
            ...
            //    props.rows
            //       .flatMap(
            //          (report: { from: number; uniquePlayers: number }) =>
            //             report?.uniquePlayers || 0
            //       )
            //       .reduce((acc: number[], curr: number, index: number) => {
            //          acc.push((acc[index - 1] || 0) + curr);
