...
import { renderMinStackCell } from '@/components/custom/PortalRenderCells'
...
import { saveLoadingOperatorBetAmountLimits, selectloadingOperatorBetAmountLimits } from 'redux/loadingSlice'
...
import { useSetOperatorBetAmountLimitsMutation } from './lib/hooks/queries'
...
export default function EditLimitsData({ id }: { id: string }) {
   ...
   const isLoading = useSelector(selectloadingOperatorBetAmountLimits)
   ...
   const columns: GridColDef[] = [
         ...
         renderCell: (params) =>
            renderMinStackCell(
               handleConvertToUSD(
                  params.row.minStack,
                  currenciesInit?.find(
                     (currency) => currency.currency === params.row.currency
                  )?.inUSD
               )
            ),
         minWidth: 100,
      ...
      {
         field: 'maxWinAmountUSD',
         align: 'center',
         headerAlign: 'center',
         headerName: 'Max Win Amount in USD',
         renderCell: (params) =>
            handleConvertToUSD(
               params.row.maxWinAmount,
               currenciesInit?.find(
                  (currency) => currency.currency === params.row.currency
               )?.inUSD
            ),
      ...
      {
         field: 'defaultBetAmountUSD',
         align: 'center',
         headerAlign: 'center',
         headerName: 'Default Bet Amount In USD',
         renderCell: (params) =>
            handleConvertToUSD(
               params.row.defaultBetAmount,
               currenciesInit?.find(
                  (currency) => currency.currency === params.row.currency
               )?.inUSD
            ),
   ...
   const { mutate } = useSetOperatorBetAmountLimitsMutation({
      onSuccess: () => {
   ...
   React.useEffect(() => {
      if (!ignore) {
         ...
         boClient?.operator.getOperatorBetAmountLimits(
            { opId: id },
            {
               uuid: uuidv4(),
               meta: {
                  ...
                  event: UserPermissionEvent.BACKOFFICE_GET_OPERATOR_BET_AMOUNT_LIMITS_REQ,
               },
            }
         )
         setIgnore(true)
