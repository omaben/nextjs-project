...
import { TournamentV2, User, UserScope } from '@alienbackoffice/back-front'
...
import { selectAuthTournamentV2IDs, selectAuthUser } from 'redux/authSlice'
import { selectLoadingTournamentV2IDs } from 'redux/loadingSlice'
import {
...
import { useGetTournamentV2IDsListQuery } from './lib/hooks/queries'

export default function AllTournamentV2IdsList(dataFilter: {
   autoRefresh: number
}) {
   ...
   const dataTournamentIDs = useSelector(selectAuthTournamentV2IDs)
   ...
   const loadingTournamentIDs = useSelector(selectLoadingTournamentV2IDs)
   ...
   useGetTournamentV2IDsListQuery(dataFilter?.autoRefresh)
   ...
   React.useEffect(() => {
      if (dataTournamentIDs && dataTournamentIDs.length > 0) {
         ...
         const fetchPromises = dataTournamentIDs.map((obj: string) =>
            fetchJsonData(`https://tournament.imoon.com/stats/${obj}-v2.json`)
