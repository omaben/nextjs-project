...
export default function PermissionsData(data: {
   ...
   const [rowCountState, setRowCountState] = React.useState(
      data.userPermissionEventInit?.filter((item: UserPermission) =>
         item.scopes?.includes(data.user.scope)
      ).length || 0
   )
      ...
      >
         <DataGridPro
            ...
            onRowSelectionModelChange={(newSelectionModel) => {
               const dataItem = data.userPermissionEventInit
                  ?.filter((item: UserPermission) =>
                     item.scopes?.includes(data.user.scope)
                  )
