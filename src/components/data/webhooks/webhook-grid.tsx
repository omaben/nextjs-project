...
import { selectLoadingWebhookList } from 'redux/loadingSlice'
import {
...
export default function WebhookData(dataFilter: UseGetWebhookListQueryProps) {
   /** start declare */
   ...
   const data = useSelector(selectAuthWebhookList)
   const [jsonData, setJsonData] = useState({} as WebhookLog)

   const [openFullScreen, setOpenFullScreen] = React.useState(false)
   const [maxWidth, setMaxWidth] = React.useState<DialogProps['maxWidth']>('xl')
   const [fullWidth, setFullWidth] = React.useState(true)
   const handleOpenFullClick = (data: WebhookLog) => {
      setJsonData(data)
   ...
   const columns: GridColDef[] = [
      ...
      },
      {
         ...
         renderCell: (params) => (
            <Box px={0.5} lineHeight={1.2}>
               <Tooltip title={params.row.opFullId || params.row.opId} placement="top">
                  <Stack>
                     <PortalCopyValue
                        value={params.row.opFullId || params.row.opId}
                        sx={{
                           '.MuiTypography-root': {
      ...
      },
      {
         ...
         headerAlign: 'center',
         align: 'center',
         renderCell: (params) => renderWebhookResultCell(params.value),
         flex: 1,
         sortable: false,
         minWidth: 155,
      },
      {
         ...
         headerAlign: 'center',
         align: 'center',
         renderCell: (params) => renderGameCell(params.value),
         flex: 1,
         sortable: false,
         minWidth: 80,
      },
      ...
      },
      {
         field: 'latency',
         headerName: 'Latency',
         renderCell: (params) => renderWebhookLatencyCell(params.value),
         flex: 1,
         sortable: false,
         minWidth: 100,
         headerAlign: 'center',
      ...
      },
      {
         ...
         headerName: 'Result',
         renderCell: (params) =>
            renderWebhookStatusCell(params.value.toLocaleUpperCase()),
         flex: 1,
         sortable: false,
         minWidth: 155,
         headerAlign: 'center',
   ...
   const loadingWebHook = useSelector(selectLoadingWebhookList)
