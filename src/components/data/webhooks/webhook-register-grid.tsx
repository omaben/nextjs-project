...
import { selectAuthList, selectAuthOperator } from 'redux/authSlice'
import { selectLoadingAuthLogList } from 'redux/loadingSlice'
import {
...
import {
   ...
   useGetRegiterListQuery,
} from './lib/hooks/queries'
export function CustomFooterStatusComponent(props: {}) {
   return <Divider></Divider>
}
export default function WebhookRegisterData(
   ...
   const post: UseGetWebhookListQueryProps = {
      ...
      authType: dataFilter.authType,
      authMethod: dataFilter.authMethod,
      refresh: dataFilter.refresh,
   }

   useGetRegiterListQuery(post)

   const data = useSelector(selectAuthList)
      ...
      },
      {
         ...
         renderCell: (params) => (
            <Box px={0.5} lineHeight={1.2}>
               <Tooltip
                  title={params.row.opFullId || params.row.opId}
                  placement="top"
               >
                  <Stack>
                     <PortalCopyValue
   ...
   const loadingWebHook = useSelector(selectLoadingAuthLogList)
