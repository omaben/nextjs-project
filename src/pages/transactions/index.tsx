...
function TransactionsList() {
   ...
   const handlelogDate = async (
      ...
      if (update) {
         router.push(
            `/transactions?type=${query.type}&from=${
   ...
   return (
      <React.Fragment>
         <Helmet
            title={`iMoon | ${
               query.type === 'all' ? 'Transaction' : query.type
            } List`}
         />
         {isDesktop ? (
            <>
               ...
               <HeaderToolbar
                  ...
                  actions={
                     <>
                        {['all', 'DEPOSIT'].includes(query.type) && (
                           <Grid item>
                              <FormControlLabel
                                 control={
                                    <Checkbox
                                       ...
                                       color="primary"
                                       onChange={
                                          handleChangeExcludePendingDeposit
                                       }
                                    />
                                 }
         ...
         {ignore ? (
            <Transactions
               ...
               excludePendingDeposit={
                  query.type === 'all' || query.type === TransactionType.DEPOSIT
                     ? excludePendingDeposit
                     : false
               }
               iban={filters.iban}
...
TransactionsList.getLayout = function getLayout(page: ReactElement) {
   return <DashboardLayout title="Transaction List">{page}</DashboardLayout>
}
